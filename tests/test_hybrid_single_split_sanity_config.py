"""Regression tests for the fixed hybrid single-split sanity config."""
...
class TestHybridSingleSplitSanityConfig(unittest.TestCase):
    def test_config_locks_the_fixed_hybrid_sanity_protocol(self) -> None:
        config_path = Path("configs/hybrid_single_split_sanity.yaml")
        with config_path.open(encoding="utf-8") as handle:
            cfg = yaml.safe_load(handle)

        self.assertEqual(cfg["model"]["type"], "hybrid")
        self.assertEqual(cfg["foundation_x"]["checkpoint_path"], "checkpoints/foundation_x.pth")
        self.assertFalse(cfg["foundation_x"]["frozen"])
        ...
        self.assertEqual(cfg["branch_input_views"]["cnn_branch"]["transform"], "identity")
        self.assertEqual(
            cfg["branch_input_views"]["foundation_x_branch"]["normalization"]["type"],
            "imagenet_mean_std",
        )
