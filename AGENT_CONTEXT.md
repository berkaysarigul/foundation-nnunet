...
- The real pretrained forward/training path on this local desktop environment, because the current runtime is CPU-only (`torch 2.11.0+cpu`, `cuda_available=False`) and is not suitable for the authoritative pretrained baseline run even though the Colab-oriented runner is now in place.
