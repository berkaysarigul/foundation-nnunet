...
- Subtasks:
  ...
  - [x] Record final policy in `DECISIONS.md`.
    ...
    - Validation note (2026-04-21): D-070 now implements the D-062 contract in the living code path. `src/models/backbone.py` now owns the Foundation X branch-specific grayscale-to-RGB repeat plus explicit ImageNet mean/std normalization, while the CNN branch remains on the raw grayscale path. `py -3 -m unittest tests.test_hybrid_gradient_flow -v` passed after adding helper-level and forward-level normalization regressions alongside the existing gradient-flow checks.
    - Validation note (2026-04-22): authoritative `run_metadata.yaml` now records hybrid `branch_input_views` explicitly through `src/training/run_artifacts.py::build_run_metadata(...)`, including the raw grayscale dataset view, the CNN identity view, and the Foundation X RGB + ImageNet mean/std view. `py -3 -m unittest tests.test_run_artifacts -v` passed after adding regression coverage for the new hybrid metadata surface.
    - Validation note (2026-04-22): the shortest-path hybrid GPU surface is now explicit on the config side too. `configs/hybrid_single_split_sanity.yaml` records the D-062 branch views directly, and `scripts/run_hybrid_single_split_sanity.py` now provides a Colab-friendly `train -> select -> test` entrypoint for the first trusted single-split hybrid sanity run. `py -3 -m unittest tests.test_hybrid_single_split_sanity_config tests.test_hybrid_single_split_sanity_runner -v` passed.
- Success criteria:
...
- Subtasks:
  ...
  - [x] Record the first executed repeated-split pretrained baseline pilot in repo memory.
    - Validation note (2026-04-22): D-071 now records the first external GPU/Colab repeated-split pretrained pilot, `resnet34_repeated_split_pilot_v1`, from the user-provided authoritative study outputs. Recorded held-out `test_dice_pos_mean` values are `split_001=0.4911`, `split_002=0.5196`, and `split_003=0.5067`; the recorded `summary/final_summary.yaml` reports mean `0.5057809632887573`, 95% CI `[0.49106974694330024, 0.5196164334743154]`, contributing split count `3`, and contributing split IDs `split_001`, `split_002`, `split_003`. `Select-String` and `git diff` were reviewed after updating the recovery-memory files to keep this pilot separate from the still-unchanged D-042 single-run Foundation X comparison anchor.
  - [x] Decide how the first repeated-split pilot is used before larger reference studies exist.
    - Validation note (2026-04-22): D-072 now accepts `resnet34_repeated_split_pilot_v1` as the interim repeated-split supervised anchor. Candidate-model repeated-split comparisons may proceed immediately, but only on the exact `split_001` / `split_002` / `split_003` instances from the pilot and only with pilot/interim framing; a later wider pretrained repeated-split reference study may supersede this anchor for publication-final claims. `Select-String` and `git diff` were reviewed to confirm the queue, agent context, and checklist all use the same interim-vs-final distinction.
- Success criteria:
